	...
	public Input
{
	...
	bool _isKeyPressed(KeyCode key) override {
		...
		int action = glfwGetKey(window, glfwKeyMap[key]);
    ...
    bool _isKeyReleased(KeyCode key) override {
        GLFWwindow* window = (GLFWwindow*)Application::getInstance()->getWindow()->getNativeWindow();
        if (!window) return false;

        int glfwKey = glfwKeyMap[key];
        ...
        if (currentlyPressed && !wasPressed) {
            // transizione da rilasciato -> premuto: singolo evento
            wasPressed = true;
            return true;
        }
